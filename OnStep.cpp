...
int OnStep::setTarget(double dRa, double dDec)
{
    ...
    // convert Ra value to HH:MM:SS.S before passing them to the OnStep
...
int OnStep::setTrackingRates(bool bSiderialTrackingOn, bool bIgnoreRates, double dRaRateArcSecPerSec, double dDecRateArcSecPerSec)
{
    ...
    m_sLogFile << "["<<getTimeStamp()<<"]"<< " [setTrackingRates] check current traking value : " << sResp << std::endl;
    m_sLogFile.flush();
#endif
...
int OnStep::startOpenLoopMove(const MountDriverInterface::MoveDir Dir, unsigned int nRate)
{
    ...
    nErr = sendCommand(sCmd, sResp, MAX_TIMEOUT, SHORT_RESPONSE, 0); // this command doesn't send any response
    if(nErr)
        return nErr;
...
int OnStep::setSiteData(double dLongitude, double dLatitute, double dTimeZone)
{
    ...
    convertDecDegToDDMM(dLongitude, sLong);
    convertDecDegToDDMM(dLatitute, sLat);
...
void OnStep::convertDecDegToDDMM(double dDeg, std::string &sResult)
{
    int DD, MM;
    double mm;
    double dNewDeg;
    ...
    sResult.clear();
    // convert dDeg decimal value to sDD:MM
	...
	ssTmp << cSign << DD << "*" << std::setfill('0') << std::setw(2) << MM;
